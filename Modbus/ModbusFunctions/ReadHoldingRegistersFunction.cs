...
namespace Modbus.ModbusFunctions
{
    ...
    public class ReadHoldingRegistersFunction : ModbusFunction
    {
        ...
        public override Dictionary<Tuple<PointType, ushort>, ushort> ParseResponse(byte[] response)
        {
            ...
            if (response[7] == CommandParameters.FunctionCode + 0x80)   //provjerava da li je doslo do greske
            {
                HandeException(response[8]);
            }
            else
            {
                ushort adresa = ((ModbusReadCommandParameters)CommandParameters).StartAddress;  //start adresa
                ushort value;   //ovde ce uvijek biti vrijednost
                for (int i = 0; i < response[8]; i = i + 2) //response[8] -> byteCount tj. koliko imamo bajtova u dijelu koji nosi vrijednosti signala
                { //i se uvecava za dva jer preuzimamo short vrijednosti
                    value = BitConverter.ToUInt16(response, (i + 9));   //prva vrijednost
                    value = (ushort)IPAddress.NetworkToHostOrder((short)value); //prelazak u host order
